...
func Load(efivars string) (*Config, error) {
	...
	for _, fp := range entry.FilePathList {
		...
		case efidevicepath.MACAddressType:
			...
			if err != nil {
				return &Config{}, fmt.Errorf("parse MAC from current load option: %w", err)
			}
		...
		case efidevicepath.VLANType:
			...
			if err != nil {
				return &Config{}, fmt.Errorf("parse VLAN from current load option: %w", err)
			}
		...
		case efidevicepath.IPv4Type:
			...
			if err != nil {
				return &Config{}, fmt.Errorf("parse IPv4 from current load option: %w", err)
			}
		...
		case efidevicepath.IPv6Type:
			...
			if err != nil {
				return &Config{}, fmt.Errorf("parse IPv6 from current load option: %w", err)
			}
		...
		case efidevicepath.DNSType:
			...
			if err != nil {
				return &Config{}, fmt.Errorf("parse DNS from current load option: %w", err)
			}
		...
		case efidevicepath.URIType:
			...
			if err != nil {
				return &Config{}, fmt.Errorf("parse URI from current load option: %w", err)
			}
